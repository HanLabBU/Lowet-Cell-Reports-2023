...
for foldn= [ 12]% 3 8]%:length(foldnames);   %% folders
    cd(mainpath)
    cd(foldnames{foldn})
    ses=dir('*.mat');
     ...
     [PLV_b]= spike_field_ppc_adj(wavD(:,:,:,:),dataM.roasterT1,CH,sp_shift) ;
       ...
       [PLV_b_SH]= spike_field_ppc_adjSHUFFLE_v2(wavD(:,:,:,:),dataM.roasterT1,CH,sp_shift) ;
                        ...
                        allaM3(:,mr)=PLV_b-PLV_b_SH;%PLV_b;
                              %  allaM3(:,mr)=PLV_s_SH;%PLV_s;
                       % allaM3B(:,mr)=PLV_s;%PLV_b;        
...
%xlim([-0.015 0.02])
 print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'OSC_opto_GAM_opt_trigSS.pdf'])
...
hold on,plot(ax,optmod.*(m-2),'k','Linewidth',2.5,'COlor',[0.3 0.8 1])
%xlim([-0.015 0.02])
 print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'OSC_opto_GAM_opt_trigBS.pdf'])
%savefig(gcf, [ savepath 'OSC_opto_Gam_opt_trigBS.fig'])
  end
% 
...
ylim([-0.6 .6]);set(gca,'Xticklabel',[],'Xtick',[])
 print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'OSC_40Hzopto_THET_PLV_BAR.pdf'])
savefig(gcf, [ savepath 'OSC_opto40HZ_THET_PLV_BAR.fig'])
...
% violinplot2(V2-V1,[1.3 ],'ViolinColor', [ 0.9 0.3 0.7; 0 0 0.9])
% %violinplot2(V2,[1.8 ],'ViolinColor', [ 0 0 0.9; 0 0 0.9])
% line([ 0.7 1.8], [ 0  0],'COlor', [ 0 0 0 ],'Linewidth',0.5)
% ylim([-0.6 .6])
%  print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'OSC40opto_GAM_PLV_BAR.pdf'])
...
axis tight;set(gca,'Xticklabel',[],'Xtick',[])
 print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'OSC_opto_40Hz_FIRING_BAR.pdf'])
