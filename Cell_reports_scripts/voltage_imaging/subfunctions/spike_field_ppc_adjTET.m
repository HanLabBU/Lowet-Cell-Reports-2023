function [ PLV_output,PLV_phase]= spike_field_ppc_adjTET(wavD,spikes,CH,sp_shift)
if size(wavD,4)>1
...
%     end
%     
    s=spikes;%find(spikes{ind});
    s=s-sp_shift; s(s<=10)=[]; % spike times, shifted to avoid spike contamination issue
    s(s> length(wavD))=[];
    mat=[mat,   wavD(:,s)];
end
